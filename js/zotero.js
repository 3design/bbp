 ...
 */
async function fetchReferences(_urls, sort="date", direction="desc", queryString) {
    const urls = _urls.map(url => new URL(url))
...
const fetchItemChildren = (itemKey) => {
    fetch(`https://api.zotero.org/groups/${ZETORO_ID}/items/${itemKey}/children?format=json&limit=2000`)
        .then(res => res.json())
        .then(data => {
